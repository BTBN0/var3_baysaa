...
import { Bell, Users, LogOut, Hash, MessageSquare, UserPlus, ChevronDown, ChevronRight, Plus, Settings } from "lucide-react";
    ...
    <div style={{ display: "flex", height: "100%" }}>
      ...
      <div style={{ width: 210, background: "var(--surface)", display: "flex", flexDirection: "column", borderRight: "1px solid var(--border)" }}>
          ...
          {tab === "friends" && (
            <div>
              <button
                ...
                onMouseEnter={(e) => e.currentTarget.style.borderColor = "var(--text5)"}
                    ...
                    <div>
                      ...
                      {showOffline && offlineFriends.map((friend) => (
                        <div key={friend.id} onClick={() => navigate(`/dm/${friend.id}`)} style={{ ...memberRowStyle, opacity: 0.5 }}
                          onMouseEnter={(e) => { e.currentTarget.style.background = "var(--surface2)"; e.currentTarget.style.opacity = 1; }}
        ...
        <div style={{ padding: "8px 10px", borderTop: "1px solid var(--border)", display: "flex", alignItems: "center", gap: 8 }}>
          ...
          <span onClick={() => navigate("/profile")} style={{ fontSize: 12, fontWeight: 500, color: "var(--text2)", flex: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", cursor: "pointer" }}>
            {user?.username}
          </span>
