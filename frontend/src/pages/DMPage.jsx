...
import { Phone, Video, Pin, CornerUpLeft, Edit2, Trash2, X, Smile } from "lucide-react";
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0 }}>
        ...
        <div style={{ height: 48, borderBottom: "1px solid var(--border)", background: "var(--surface)", display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 16px", flexShrink: 0 }}>
          ...
          <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
            {canMsg && (<>
              <HeaderBtn onClick={() => navigate(`/call/${userId}?mode=call&video=0`)} accent="green" title="Залгах"><Phone size={13} /></HeaderBtn>
              <HeaderBtn onClick={() => navigate(`/call/${userId}?mode=call&video=1`)} title="Видео"><Video size={13} /></HeaderBtn>
            </>)}
            <HeaderBtn onClick={() => setBlockModal(true)} accent="red" title={isBlocked ? "Unblock" : "Block"}>🚫</HeaderBtn>
          </div>
        </div>
        ...
        {replyTo && (
          <div style={{ padding: "5px 16px", borderTop: "1px solid var(--border)", background: "var(--surface2)", display: "flex", alignItems: "center", gap: 8 }}>
            <CornerUpLeft size={13} color="var(--text5)" />
            ...
            <button onClick={() => setReplyTo(null)} style={{ background: "none", border: "none", cursor: "pointer", color: "var(--text5)", display: "flex", alignItems: "center" }}><X size={13} /></button>
          </div>
        )}
...
function HeaderBtn({ onClick, title, accent, children }) {
  ...
  return (
    <button onClick={onClick} title={title} onMouseEnter={() => setH(true)} onMouseLeave={() => setH(false)} style={{
      padding: "5px 10px", borderRadius: 8, border: "1px solid var(--border)", cursor: "pointer", transition: "all .15s",
      display: "flex", alignItems: "center", gap: 5, fontSize: 12, fontWeight: 600,
      background: h ? (accent==="green" ? "#16a34a" : accent==="red" ? "rgba(239,68,68,.85)" : "var(--surface2)") : "transparent",
      color: h ? (accent==="green" || accent==="red" ? "#fff" : "var(--text)") : "var(--text5)",
    }}>{children}</button>
  );
}
      ...
      <div onMouseEnter={() => setHover(true)} onMouseLeave={() => { setHover(false); setShowEmoji(false); }} style={{
        ...
        {hover && !msg.deleted && !editing && (
          <div style={{ position: "absolute", right: 8, top: showAvatar ? 6 : -10, display: "flex", gap: 2, background: "var(--surface)", border: "1px solid var(--border2)", borderRadius: 8, padding: "3px 4px", boxShadow: "0 4px 12px rgba(0,0,0,.25)", zIndex: 10 }}>
            {[
              { icon: <Smile size={12} />, fn: () => setShowEmoji(p => !p), title: "Emoji", cond: true, isEmoji: true },
              { icon: <CornerUpLeft size={12} />, fn: onReply,     title: "Хариулах",  cond: true },
              { icon: <Pin         size={12} />, fn: onPin,       title: msg.pinned ? "Тогтоолтыг арилгах" : "Тогтоох", cond: true },
              { icon: <Edit2       size={12} />, fn: () => { setEditVal(msg.content || ""); setEditing(true); }, title: "Засах",    cond: isOwn },
              { icon: <Trash2      size={12} />, fn: onDelete,    title: "Устгах",   cond: isOwn, danger: true },
